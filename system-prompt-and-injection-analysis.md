...
### 2.1 核心入口：`getSystemPrompt()`

定义在 `src/constants/prompts.ts`，这是 system prompt 的**唯一构建入口**：
...
返回值为 `string[]`，每个元素代表一个"段落"（section），最终通过 `join('\n\n')` 拼接。

### 2.2 完整内容结构

System prompt 由**静态区**和**动态区**两部分组成，以 `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 为分界线：

#### 静态区（Static — 可跨组织缓存 scope: 'global'）

| 序号 | Section | 函数 | 内容摘要 |
|------|---------|------|----------|
| 1 | Intro | `getSimpleIntroSection()` | 身份定义 + 安全指令（CYBER_RISK_INSTRUCTION） |
| 2 | System | `getSimpleSystemSection()` | Markdown 渲染、权限模式、系统提醒、hooks、自动压缩 |
| 3 | Doing Tasks | `getSimpleDoingTasksSection()` | 编码规范、代码风格、安全要求、反过度设计 |
| 4 | Executing Actions | `getActionsSection()` | 行动审慎原则、风险确认、不可逆操作保护 |
| 5 | Using Your Tools | `getUsingYourToolsSection()` | 工具使用优先级、并行调用指导 |
| 6 | Tone and Style | `getSimpleToneAndStyleSection()` | 无 emoji、简洁、代码引用格式 |
| 7 | Output Efficiency | `getOutputEfficiencySection()` | 输出简洁性要求 |

**各 Section 核心内容详解**：

**`getSimpleIntroSection()`** — 身份定义与安全边界：
...
- Markdown 渲染规则（Github-flavored, CommonMark monospace）
- 权限模式说明：工具调用需用户审批
- `<system-reminder>` 标签说明（系统自动注入，与具体工具结果无直接关系）
- 外部数据注入警告（prompt injection 防护）
- Hooks 机制说明（`<user-prompt-submit-hook>` 视为用户输入）
- 自动压缩说明（上下文无限，通过自动摘要实现）

**`getSimpleDoingTasksSection()`** — 任务执行规范：

- 不要过度设计（No gold-plating, no unnecessary error handling）
- 不要过早抽象（Three similar lines of code > premature abstraction）
- 先读代码再改代码（`In general, do not propose changes to code you haven't read`）
- 不要创建不必要的文件
- 安全编码要求（OWASP Top 10）
- Ant-only 条目：注释规范、真实性报告（拒绝虚假声明）、Bug 报告引导（`/issue` 或 `/share`）

**`getActionsSection()`** — 行动审慎原则：

- 谨慎评估行动的可逆性和影响范围
- 不可逆操作必须确认（git push、删除文件、修改 CI/CD 等）
- 不要用破坏性操作绕过障碍（不要 `--no-verify`，不要直接删 lock 文件）
- 一次审批 ≠ 永久授权（`A user approving an action once does NOT mean they approve it in all contexts`）

**`getUsingYourToolsSection()`** — 工具使用指导：

- 专用工具优先（Read > cat, Edit > sed, Write > heredoc, Glob > find, Grep > grep）
- Bash 仅用于系统命令
- 独立工具调用可并行，依赖调用须顺序
- TodoWrite/TaskCreate 用于任务规划
...
- 外部用户版：简短直接，直奔主题
- Ant-only 版：详细的沟通规范（prose 写作、避免语义回溯、倒金字塔结构）

#### 动态区（Dynamic — 会话/用户特定，scope: null）

| 序号 | Section | 函数 | 内容摘要 | 缓存策略 |
|------|---------|------|----------|----------|
| D1 | Session Guidance | `getSessionSpecificGuidanceSection()` | Agent/Fork 工具、Skill、Explore Agent、Verification Agent | 缓存 |
| D2 | Memory | `loadMemoryPrompt()` | 自动记忆/团队记忆 | 缓存 |
| D3 | Ant Model Override | `getAntModelOverrideSection()` | 模型覆盖提示 | 缓存 |
| D4 | Env Info | `computeSimpleEnvInfo()` | 工作目录、Git、OS、模型ID、知识截止日 | 缓存 |
| D5 | Language | `getLanguageSection()` | 语言偏好 | 缓存 |
| D6 | Output Style | `getOutputStyleSection()` | 输出风格配置 | 缓存 |
| D7 | MCP Instructions | `getMcpInstructionsSection()` | MCP 服务器指令 | **不缓存** ⚠️ |
| D8 | Scratchpad | `getScratchpadInstructions()` | 临时文件目录 | 缓存 |
| D9 | FRC | `getFunctionResultClearingSection()` | 工具结果自动清除 | 缓存 |
| D10 | Summarize | — | 工具结果摘要提醒 | 缓存 |
| D11 | Length Anchors | — | 数值长度锚点（Ant-only） | 缓存 |
| D12 | Token Budget | — | Token 目标预算 | 缓存 |
| D13 | Brief | — | Brief 工具指令 | 缓存 |
...
**MCP Instructions 是唯一使用 `DANGEROUS_uncachedSystemPromptSection` 的动态段**，因为 MCP 服务器可以在会话中途连接/断开。当 `isMcpInstructionsDeltaEnabled()` 开启时，MCP 指令改为通过增量附件注入，避免每轮重算 system prompt。
...
| 文件 | 行号 | 内容 |
|------|------|------|
| `src/constants/prompts.ts` | 444-577 | `getSystemPrompt()` 主入口 |
| `src/constants/prompts.ts` | 175-253 | 各静态 section 函数定义 |
| `src/constants/prompts.ts` | 606-710 | `computeSimpleEnvInfo()` 环境信息 |
| `src/constants/prompts.ts` | 760-791 | `enhanceSystemPromptWithEnvDetails()` 子 agent prompt |
| `src/constants/system.ts` | 10-95 | Identity prefix 与 Attribution header |
| `src/constants/systemPromptSections.ts` | 1-68 | Section 缓存机制 |
| `src/constants/cyberRiskInstruction.ts` | 1-24 | 安全指令 |
| `src/utils/systemPrompt.ts` | 41-123 | `buildEffectiveSystemPrompt()` 优先级链 |
| `src/utils/systemPromptType.ts` | 1-14 | `SystemPrompt` 类型定义 |
| `src/utils/claudemd.ts` | 790-1075 | `getMemoryFiles()` 主流程 |
| `src/utils/claudemd.ts` | 1153-1195 | `getClaudeMds()` 渲染格式 |
| `src/utils/claudemd.ts` | 254-279 | Frontmatter glob 解析 |
| `src/utils/claudemd.ts` | 443-535 | @include 指令提取 |
| `src/utils/attachments.ts` | 743-1003 | `getAttachments()` 附件计算 |
| `src/utils/attachments.ts` | 440-720 | Attachment 类型定义 |
| `src/utils/attachments.ts` | 2937-2970 | `getAttachmentMessages()` 生成器 |
| `src/utils/attachments.ts` | 3201-3210 | `createAttachmentMessage()` |
| `src/utils/attachments.ts` | 1455-1559 | Delta 附件函数 |
| `src/utils/api.ts` | 321-410 | `splitSysPromptPrefix()` 缓存分割 |
| `src/utils/api.ts` | 437-474 | `appendSystemContext()` / `prependUserContext()` |
| `src/context.ts` | 116-189 | `getSystemContext()` / `getUserContext()` |
| `src/services/api/claude.ts` | 1358-1379 | System prompt 最终组装 |
| `src/services/api/claude.ts` | 3213-3237 | `buildSystemPromptBlocks()` |
| `src/tools.ts` | 193-251 | `getAllBaseTools()` 工具清单 |
| `src/tools.ts` | 345-367 | `assembleToolPool()` 工具池组装 |
| `src/screens/REPL.tsx` | 2535-2565 | System prompt 初始化入口 |
| `src/memdir/memdir.ts` | 419-500 | `loadMemoryPrompt()` 记忆加载 |
